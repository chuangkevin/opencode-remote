...
- Updater: hourly and at load via `io.interagent.opencode-sara-updater`; installed script `/Users/kevin/.local/share/opencode-remote/update-opencode-sara.sh`
- Source artifacts: `deploy/macos/run-opencode-sara.sh`, `deploy/macos/update-opencode-sara.sh`, both LaunchAgent plists, and `deploy/macos/deploy-local.sh`
...
Full Disk Access must be granted to the actual binaries resolved by the stable Homebrew paths `/opt/homebrew/bin/node` and `/opt/homebrew/bin/opencode`. Discover the current OpenCode target with `node -p 'fs.realpathSync("/opt/homebrew/bin/opencode")'`; do not hard-code a versioned internal Homebrew layout. Deployment creates `/Users/kevin/Documents/Projects/.opencode-remote/remote-fda-probe.txt` with fixed non-secret ASCII content and mode `0644`, and requires the newly launched OpenCode API to read it through the Remote proxy. Homebrew upgrades may replace the resolved binary, so regrant FDA if it changed. The GUI launchd domain may contain unrelated inherited variables, so do not share raw `launchctl print` output. The wrapper does not source `.env`. No Basic auth is acceptable only because the listener is pinned to the trusted Tailnet address. Do not add public exposure or change the macOS bind to `0.0.0.0` without a separate security review.
...
After the main-service health and FDA probe gate passes, the installer idempotently replaces only `gui/$UID/io.interagent.opencode-sara-updater`, clears any updater block marker, and bootstraps the updater. The updater runs hourly and at load under a clean fixed environment. Strict status requires Remote and every available live Desktop source to succeed and be idle; a closed Desktop has no live source and does not block an update. After an update is found it atomically owns a quiesce marker, waits one second, and checks strict status again. While quiesced, only new Remote prompt-creating POSTs return retryable `503`; Desktop is not restarted. The formula-scoped Homebrew check accepts only its exact exit/stdout contract. Upgrade keeps the old keg, restarts only `io.interagent.opencode-sara`, and requires exact new health version plus FDA probe. Any post-upgrade failure writes a persistent block marker and attempts to restore only the recorded `/opt/homebrew/bin/opencode` symlink target, then verifies old health plus FDA. Hourly runs defer while blocked until a successful manual deploy. A recovered update closes only the maintenance ID returned by its own POST; rollback failure leaves it to expire. Logs are `opencode-sara-updater.log` and `opencode-sara-updater.error.log`; manual execution uses `/Users/kevin/.local/share/opencode-remote/update-opencode-sara.sh`.
...
- macOS wrapper 可設定 `OPENCODE_UPDATE_QUIESCE_FILE`。marker 存在時只拒絕 session `message` 與 `prompt_async` POST，回 `503` JSON、`Retry-After: 1`、`no-store`；abort、session creation、pins、health 與 status 照常。
